...
const filter = reactive({
  from: '', to: '', groupId: '', featureKey: '' as FeatureKey | '',
  result: '' as AnalyticsResultFilter | '', metric: 'group_message_count' as AnalyticsMetricKey,
...
const metricOptions: { value: AnalyticsMetricKey; label: string }[] = [
  ...
  { value: 'ai_duration_ms', label: 'AI 耗时' },
...
function hydrateFromRoute(): void {
  ...
  filter.from = (queryString(route.query.from) || `${defaults.from}T00:00:00Z`).slice(0, 10)
  filter.to = (queryString(route.query.to) || `${defaults.to}T23:59:59Z`).slice(0, 10)
  filter.groupId = queryString(route.query.group_id)
  filter.featureKey = queryString(route.query.feature_key) as FeatureKey | ''
  filter.result = queryString(route.query.result) as AnalyticsResultFilter | ''
  filter.metric = (queryString(route.query.metric) || 'group_message_count') as AnalyticsMetricKey
  filter.dimension = (queryString(route.query.dimension) || 'group') as AnalyticsDimension
  filter.granularity = (queryString(route.query.granularity) || 'day') as AnalyticsGranularity
}

const commonQuery = computed(() => ({
  from: `${filter.from}T00:00:00Z`,
  to: `${filter.to}T00:00:00Z`,
  groupIds: filter.groupId ? [filter.groupId] : [],
  featureKeys: filter.featureKey ? [filter.featureKey] : [],
...
async function load(): Promise<void> {
  ...
  error.value = null
  try {
    const [nextSummary, nextTimeseries, nextRankings] = await Promise.all([
      analyticsApi.getSummary(commonQuery.value),
...
async function applyFilters(): Promise<void> {
  await router.replace({ query: {
    ...
    feature_key: filter.featureKey || undefined,
...
watch(() => route.fullPath, () => { hydrateFromRoute(); void load() }, { immediate: true })
</script>
...
<template>
  <main class="analytics-page">
    ...
    <form data-test="analytics-filters" class="analytics-filters" @submit.prevent="applyFilters">
      <label><span>开始日期</span><input v-model="filter.from" name="from" type="date" required /></label>
      <label><span>结束日期</span><input v-model="filter.to" name="to" type="date" required /></label>
      <label><span>群号</span><input v-model.trim="filter.groupId" name="group_id" inputmode="numeric" placeholder="全部群" /></label>
      <label><span>功能</span><select v-model="filter.featureKey" name="feature_key"><option value="">全部功能</option><option value="ai_qa">AI 问答</option><option value="quote">引用图</option><option value="link_cleaner">链接净化</option><option value="custom_commands">自定义命令</option></select></label>
      <label><span>结果</span><select v-model="filter.result" name="result"><option value="">全部结果</option><option value="success">成功</option><option value="failed">失败</option><option value="fallback">降级</option><option value="denied">拒绝</option><option value="unknown">未知</option><option value="skipped">跳过</option></select></label>
      <label><span>指标</span><select v-model="filter.metric" name="metric"><option v-for="item in metricOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
      <label><span>排行维度</span><select v-model="filter.dimension" name="dimension"><option value="group">群</option><option value="command">命令</option><option value="knowledge_entry">知识词条</option></select></label>
      <label><span>粒度</span><select v-model="filter.granularity" name="granularity"><option value="day">按日</option><option value="hour">按小时</option></select></label>
    ...
    <ResourceState v-if="loading && !summary" state="loading" title="正在汇总统计" description="正在读取指标、趋势和排行。" />
    <ResourceState v-else-if="error && !summary" state="error" title="统计读取失败" description="筛选条件已保留，可以重新尝试。" @retry="load" />

    <template v-else-if="summary">
      <div v-if="error" class="stale-state"><RefreshCw :size="15" />刷新失败，当前仍显示上一次成功数据。</div>
      ...
      <section class="analytics-workspace">
        <section class="trend-section">
          <header><div><h2>趋势</h2><p>{{ metricOptions.find((item) => item.value === filter.metric)?.label }} · {{ filter.granularity === 'day' ? '按日' : '按小时' }}</p></div><TrendingUp :size="18" /></header>
          <AnalyticsTrendChart :series="timeseries?.series ?? []" />
        </section>
        <section class="ranking-section">
          <header><div><h2>排行</h2><p>{{ filter.dimension === 'group' ? '群' : filter.dimension === 'command' ? '命令' : '知识词条' }}维度前 10</p></div></header>
          <RankingTable :rankings="rankings" />
        </section>
      </section>

      <footer class="data-freshness">数据更新于 {{ timeFormatter.format(new Date(summary.data_fresh_at)) }}<span v-if="loading"> · 正在刷新</span></footer>
    </template>
  </main>
</template>
