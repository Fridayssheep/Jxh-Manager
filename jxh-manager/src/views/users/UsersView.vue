...
type ConfirmAction =
  'reset_password' | 'disable_user' | 'enable_user' | 'revoke_user_sessions' | 'revoke_session'
type UserFormErrors = {
  username: string
  displayName: string
  qqUserId: string
...
const userForm = reactive({
  username: '',
  displayName: '',
  role: 'maintainer' as AdminRole,
  qqUserId: '',
  password: '',
})
const userFormErrors = reactive<UserFormErrors>({
  username: '',
  displayName: '',
  qqUserId: '',
  password: '',
...
const roleOptions: readonly AppSelectOption[] = Object.entries(roleLabels).map(
...
const confirmTitle = computed(() => {
  const titles: Record<ConfirmAction, string> = {
    ...
    disable_user: '停用管理账号',
    enable_user: '重新启用账号',
    revoke_user_sessions: '撤销账号全部会话',
    revoke_session: '撤销指定会话',
  }
  return confirmAction.value ? titles[confirmAction.value] : ''
})
const confirmDescription = computed(() => {
  if (confirmAction.value === 'reset_password')
    return `新密码生效后，${pendingUser.value?.display_name} 的全部会话会立即撤销。`
  if (confirmAction.value === 'disable_user')
    return `${pendingUser.value?.display_name} 将不能继续登录，已有会话仍应单独撤销。`
  if (confirmAction.value === 'enable_user')
    return `${pendingUser.value?.display_name} 将恢复登录权限。`
  if (confirmAction.value === 'revoke_user_sessions')
    return `${pendingUser.value?.display_name} 的所有管理端会话都会失效。`
...
function userListQuery(cursor: string | null): AdminUserListQuery {
  return {
    query: userFilters.query.trim(),
    role: userFilters.role,
    enabled: userFilters.enabled === '' ? null : userFilters.enabled === 'true',
    cursor,
  }
}
...
function sessionListQuery(cursor: string | null): AdminSessionListQuery {
  return {
    userId: sessionFilters.userId.trim(),
    status: sessionFilters.status,
    current: sessionFilters.current === '' ? null : sessionFilters.current === 'true',
    cursor,
  }
}

function setUserRoleFilter(value: string): void {
  userFilters.role = value as AdminRole | ''
}
function setUserEnabledFilter(value: string): void {
  userFilters.enabled = value as '' | 'true' | 'false'
}
function setSessionStatus(value: string): void {
  sessionFilters.status = value as SessionStatus | ''
}
function setCurrentSession(value: string): void {
  sessionFilters.current = value as '' | 'true' | 'false'
}
function setUserRole(value: string): void {
  userForm.role = value as AdminRole
}

function clearUserFormErrors(): void {
...
function validateUserForm(): boolean {
  ...
  const qqUserId = userForm.qqUserId.trim()
  const passwordLength = [...userForm.password].length

  if (!editingUser.value && !username) userFormErrors.username = '请输入用户名。'
  else if (!editingUser.value && !/^[a-z][a-z0-9_.-]{2,31}$/.test(username)) {
    userFormErrors.username =
      '用户名需以小写字母开头，并使用 3–32 位小写字母、数字、点、短横线或下划线。'
  }
  if (!displayName) userFormErrors.displayName = '请输入显示名称。'
  else if ([...displayName].length > 64) userFormErrors.displayName = '显示名称不能超过 64 个字符。'
  if (qqUserId && (!/^\d+$/.test(qqUserId) || qqUserId.length > 32)) {
    userFormErrors.qqUserId = 'QQ 维护身份只能填写不超过 32 位的数字。'
  }
  if (!editingUser.value && !userForm.password) userFormErrors.password = '请输入初始密码。'
  else if (!editingUser.value && (passwordLength < 12 || passwordLength > 128)) {
    userFormErrors.password = '初始密码需为 12–128 个字符。'
  }
...
async function loadSessions(reset = true): Promise<void> {
  ...
  error.value = null
  try {
    const page = await usersApi.listSessions(
      sessionListQuery(reset ? null : sessionsNextCursor.value),
    )
...
async function openEdit(user: AdminUser): Promise<void> {
  ...
  try {
    ...
    operationResult.value =
      reason instanceof AdminApiError ? reason.message : '账号详情读取失败，未打开编辑器。'
  } finally {
  ...
  try {
    ...
    if (editingUser.value) {
      ...
      const payload: AdminUserCreateRequest = {
        ...
        display_name: userForm.displayName.trim(),
        role: userForm.role,
        qq_user_id: userForm.qqUserId.trim() || undefined,
    ...
    userFormErrors.form =
      reason instanceof AdminApiError ? reason.message : '账号保存失败，请稍后重试。'
  } finally {
    saving.value = false
  }
}
...
async function confirmUserAction(): Promise<void> {
  ...
  try {
    ...
    if (reason instanceof TypeError) {
      ...
      operationTone.value =
        reason instanceof AdminApiError && reason.status === 409 ? 'warning' : 'danger'
...
<template>
  <main class="users-page">
    <header class="page-header">
      <div>
        <h1>账号与会话</h1>
        <p>管理后台身份、角色、登录状态和浏览器会话。</p>
      </div>
      <button data-test="create-user" class="primary-action" type="button" @click="openNew">
        <Plus :size="16" />新建账号
      </button>
    </header>
    ...
    <div v-rise-on-change="activeTab" class="tab-content">
      <template v-if="activeTab === 'users'">
        <form data-test="user-filters" class="filter-bar" @submit.prevent="loadUsers()">
          <label class="search-field"
            ><Search :size="15" /><span class="sr-only">搜索账号</span
            ><input v-model="userFilters.query" placeholder="用户名、显示名称或完整 QQ"
          /></label>
          <label
            ><span class="sr-only">账号角色</span
            ><AppSelect
              :model-value="userFilters.role"
              :options="roleFilterOptions"
              accessible-name="账号角色"
              data-test="users-role-filter"
              @update:model-value="setUserRoleFilter"
          /></label>
          <label
            ><span class="sr-only">账号状态</span
            ><AppSelect
              :model-value="userFilters.enabled"
              :options="accountStatusOptions"
              accessible-name="账号状态"
              data-test="users-enabled-filter"
              @update:model-value="setUserEnabledFilter"
          /></label>
        ...
        <ResourceState
          ...
          state="loading"
          title="正在读取管理账号"
          description="正在同步角色、状态和最近登录时间。"
        />
        <ResourceState
          v-else-if="error"
          state="error"
          title="账号列表读取失败"
          description="筛选条件已保留，可以直接重试。"
          @retry="loadUsers()"
        />
        <ResourceState
          ...
          state="empty"
          title="没有符合条件的账号"
          description="调整筛选或创建一个新的管理账号。"
        />
        <section v-else class="directory" aria-label="管理账号列表">
          <div class="user-grid table-heading" aria-hidden="true">
            <span>账号</span><span>角色</span><span>QQ 身份</span><span>最近登录</span
          ...
          <article v-for="user in users" :key="user.user_id" class="user-grid table-row">
            <div class="user-identity">
              ...
              <div>
                ...
                ><small class="mono">@{{ user.username }} · {{ user.user_id }}</small>
              </div>
            </div>
            <span data-label="角色" class="role-badge"
              ><ShieldCheck :size="13" />{{ roleLabels[user.role] }}</span
            >
            <span data-label="QQ 身份" class="mono">{{ user.qq_user_id || '未绑定' }}</span>
            <time data-label="最近登录">{{ displayTime(user.last_login_at) }}</time>
            <span data-label="状态" :class="['status-badge', { disabled: !user.enabled }]">{{
              user.enabled ? '正常' : '已停用'
            }}</span>
            <div class="row-actions">
              <button
                ...
                type="button"
                title="编辑账号"
                :disabled="loadingDetailId === user.user_id"
                @click="openEdit(user)"
              >
                <RefreshCw v-if="loadingDetailId === user.user_id" class="spin" :size="15" /><Pencil
                  v-else
                  :size="15"
                />
              </button>
              <button
                :data-test="`reset-password-${user.user_id}`"
                type="button"
                title="重置密码"
                @click="askUserAction('reset_password', user)"
              >
                <KeyRound :size="15" />
              </button>
              <button
                :data-test="`revoke-user-sessions-${user.user_id}`"
                type="button"
                title="撤销全部会话"
                @click="askUserAction('revoke_user_sessions', user)"
              >
                <LogOut :size="15" />
              </button>
              <button
                v-if="user.enabled"
                :data-test="`disable-user-${user.user_id}`"
                class="danger"
                type="button"
                title="停用账号"
                @click="askUserAction('disable_user', user)"
              >
                <Ban :size="15" />
              </button>
              <button
                v-else
                :data-test="`enable-user-${user.user_id}`"
                type="button"
                title="启用账号"
                @click="askUserAction('enable_user', user)"
              >
                <RefreshCw :size="15" />
              </button>
            </div>
        ...
        >
          <RefreshCw :class="{ spin: loadingMore }" :size="15" />{{
            loadingMore ? '正在读取' : '加载更多账号'
          }}
        </button>
      </template>
      ...
      <template v-else>
        <form
          data-test="session-filters"
          class="filter-bar session-filters"
          @submit.prevent="loadSessions()"
        >
          <label
            ><span class="sr-only">账号 ID</span
            ><input v-model="sessionFilters.userId" placeholder="账号 ID"
          /></label>
          <label
            ><span class="sr-only">会话状态</span
            ><AppSelect
              :model-value="sessionFilters.status"
              :options="sessionStatusOptions"
              accessible-name="会话状态"
              name="session_status"
              data-test="users-session-status"
              @update:model-value="setSessionStatus"
          /></label>
          <label
            ><span class="sr-only">当前会话</span
            ><AppSelect
              :model-value="sessionFilters.current"
              :options="currentSessionOptions"
              accessible-name="当前会话"
              data-test="users-current-session"
              @update:model-value="setCurrentSession"
          /></label>
          <button class="filter-submit" type="submit">应用筛选</button>
          <button
            class="filter-reset"
        ...
        <ResourceState
          ...
          state="loading"
          title="正在读取登录会话"
          description="不读取 Cookie 或任何会话令牌。"
        />
        <ResourceState
          v-else-if="error"
          state="error"
        ...
        <ResourceState
          ...
          state="empty"
          title="没有符合条件的会话"
          description="调整账号、状态或当前会话筛选。"
        />
        <section v-else class="directory" aria-label="登录会话列表">
          <div class="session-grid table-heading" aria-hidden="true">
            <span>会话与账号</span><span>客户端</span><span>IP 地址</span><span>最近活动</span
            ><span>状态</span><span>操作</span>
          </div>
          <article
            v-for="session in sessions"
            :key="session.session_id"
            class="session-grid table-row"
          >
            <div class="session-identity">
              ...
              <div>
                ...
                ><small class="mono"
                  >{{ session.user_id }}<b v-if="session.current"> · 当前会话</b></small
                >
              </div>
            </div>
            ...
            <span
              data-label="状态"
              :class="['status-badge', { disabled: session.status !== 'active' }]"
              >{{ sessionStatusLabels[session.status] }}</span
            >
            <div class="row-actions">
              <button
                v-if="session.status === 'active'"
                :data-test="`revoke-session-${session.session_id}`"
                class="danger"
                type="button"
                title="撤销会话"
                @click="askSessionRevoke(session)"
              >
    ...
    <AppOverlayTransition :show="editorOpen" variant="drawer">
      <div class="drawer-layer" @mousedown.self="editorOpen = false">
        ...
        >
          <header>
            <div>
              <span class="eyebrow">ADMIN IDENTITY</span>
              <h2 id="user-editor-title">{{ editingUser ? '编辑管理账号' : '新建管理账号' }}</h2>
              <p>
                {{ editingUser ? `资源版本 ${editingUser.version}` : '创建后即可使用后台登录' }}
              </p>
            </div>
          ...
          <form id="user-editor-form" class="editor-body" novalidate @submit.prevent="saveUser">
            <div v-if="userFormErrors.form" data-test="user-editor-error">
              <OperationNotice :message="userFormErrors.form" tone="danger" :closable="false" />
            </div>
            <label
              ><span>用户名</span
              ><input
              ...
              /><small id="user-username-error" class="field-error" aria-live="polite">{{
                userFormErrors.username
              }}</small></label
            >
            <label
              ...
              ><input
                ...
                aria-describedby="user-display-name-error"
              /><small id="user-display-name-error" class="field-error" aria-live="polite">{{
                userFormErrors.displayName
              }}</small></label
            >
            <label
              ><span>角色</span
            ...
            /></label>
            <label
              ><span>QQ 维护身份（可选）</span
              ><input
                id="user-qq"
                v-model="userForm.qqUserId"
                data-test="user-qq"
                name="qq_user_id"
                inputmode="numeric"
                maxlength="32"
                autocomplete="off"
                :aria-invalid="Boolean(userFormErrors.qqUserId)"
                aria-describedby="user-qq-error"
              /><small id="user-qq-error" class="field-error" aria-live="polite">{{
                userFormErrors.qqUserId
              }}</small></label
            >
            <label v-if="!editingUser"
              ...
              /><small id="user-password-error" class="field-error" aria-live="polite">{{
            ...
            <div class="role-note">
              <ShieldCheck :size="16" />
              <p>
                <strong>{{ roleLabels[userForm.role] }}</strong
                ><span v-if="userForm.role === 'super_admin'">可管理账号、会话和系统危险动作。</span
                ><span v-else-if="userForm.role === 'maintainer'"
                  >可处理日常运营，但不能管理后台账号。</span
                ><span v-else>仅查看授权范围内的脱敏数据。</span>
              </p>
            </div>
          </form>
          <footer>
      ...
      <div class="dialog-layer" role="presentation">
        ...
        >
          <header>
            ...
            <div>
              <h2 id="confirm-user-title">{{ confirmTitle }}</h2>
              <p>{{ confirmDescription }}</p>
            </div>
          </header>
          <label v-if="confirmAction === 'reset_password'"
            ><span>新密码</span
            ><input
              v-model="newPassword"
          ...
          <footer>
            ...
            ><button
              data-test="confirm-user-action"
              :class="{ 'danger-action': confirmAction !== 'enable_user' }"
              type="button"
              :disabled="confirming"
              @click="confirmUserAction"
            >
              {{ confirming ? '正在处理' : '确认操作' }}
            </button>
          </footer>
        </section>
      </div>
...
.filter-bar {
  display: grid;
  grid-template-columns: minmax(220px, 1fr) 140px 120px auto 38px;
  gap: 7px;
  padding: 10px 0;
  border-bottom: 1px solid var(--color-border);
}
...
.session-grid {
  display: grid;
  grid-template-columns: minmax(190px, 1.25fr) 125px 120px 145px 75px minmax(170px, auto);
  gap: 12px;
  align-items: center;
}
.session-grid {
  grid-template-columns: minmax(190px, 1fr) minmax(180px, 1.2fr) 120px 145px 75px 52px;
}
.table-heading {
  min-height: 36px;
  padding: 0 13px;
...
.user-editor > footer .save-action {
  color: white;
  background: var(--color-brand-action);
...
.editor-body input[aria-invalid='true'] {
  border-color: var(--color-danger);
}
.field-error {
  min-height: 15px;
...
@media (max-width: 1100px) {
  .user-grid {
    grid-template-columns: minmax(190px, 1.25fr) 125px 130px 75px minmax(170px, auto);
  }
  .user-grid > *:nth-child(3) {
    display: none;
  }
  .session-grid {
    grid-template-columns: minmax(190px, 1fr) minmax(180px, 1.2fr) 145px 75px 52px;
  }
