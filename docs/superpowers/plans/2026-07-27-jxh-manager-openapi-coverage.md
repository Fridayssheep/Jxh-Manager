...
- 外部副作用：审批、正式/测试定时发送、群同步、NapCat 重启和知识库重载均先持久化请求、运行或幂等状态，再执行外部调用；超时/断线结果进入 unknown 或 failed，并在启动时恢复中断状态。正式定时发送的明确失败允许新尝试，成功或未知结果只重试终态持久化，不盲目重发。
...
- 真实 MySQL：`go test -race -count=1 ./internal/database` 与 `go test -race -count=1 ./internal/storage` 全部通过，覆盖 001-009 迁移恢复、管理事务、正式定时运行重试、业务遥测聚合与导出；测试后两类临时 schema 残留数均为 0。
