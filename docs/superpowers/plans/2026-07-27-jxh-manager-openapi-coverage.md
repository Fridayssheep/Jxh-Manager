...
- 运营资源：`internal/joinrequests`、`internal/scheduledjobs`、`internal/scheduler`、`internal/customcommand`、`internal/telemetry`、`internal/analytics` 及 `manager_operations.go` 实现审批状态机、定时任务正式/测试运行历史、受控命令、批量遥测、统计与流式导出；AI、引用图、人工/自动决策和正式定时运行均接入非阻塞结构化遥测。
...
- 真实 MySQL：`go test -race -count=1 ./internal/database`（352.010s）与 `go test -race -count=1 ./internal/storage`（126.790s）分别通过，覆盖 001-009 迁移恢复、管理事务、正式/测试定时运行重试、审批与知识库终态恢复、自定义命令 JSON 幂等重放、业务遥测聚合与导出；测试后两类临时 schema 残留数均为 0。
