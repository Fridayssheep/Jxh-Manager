...
- 当前 Bot 证据提交：`b17d09c`
- 自动校验：`Resource/Jxh-Go/internal/management/api/management_test.go` 会核对 57 个 operation 的 method/path、唯一 operationId、状态和实际注册路由。
...
- 路由与协议：`internal/management/api` 注册完整的 57 条路由，统一执行会话认证、RBAC、CSRF、Origin、请求体与非阻塞并发限制、错误格式和请求 ID；各 handler 测试覆盖成功、未认证、无权限与关键输入边界。
- 认证与审计：`internal/management/auth`、`internal/management/audit` 和 `internal/platform/storage/manager_auth.go` 实现账号、会话、密码变更、管理员管理、审计、稳定游标和持久化幂等；真实 MySQL 测试覆盖并发与事务回滚。
- 核心资源：`internal/groups`、`internal/management/settings`、`internal/management/overview`、`internal/management/system` 及 `manager_core_*.go` 实现群目录、全局/群设置、总览和系统健康/重启；设置热更新不进入消息热路径查询数据库。
- 运营资源：`internal/groups/joinrequests`、`internal/automation/scheduledjobs`、`internal/automation/scheduler`、`internal/automation/customcommand`、`internal/platform/telemetry`、`internal/management/analytics` 及 `manager_operations.go` 实现审批状态机、定时任务正式/测试运行历史、受控命令、批量遥测、统计与流式导出；AI、引用图、人工/自动决策和正式定时运行均接入非阻塞结构化遥测。
- 外部副作用：审批、正式/测试定时发送、群同步、NapCat 重启和知识库重载均先持久化请求、运行或幂等状态，再执行外部调用；超时/断线结果进入 unknown 或 failed，并在启动时恢复中断状态。外部调用完成但终态落库失败时，服务只重试同一终态载荷，不重复审批、发送、同步、重启或重载；正式定时发送的明确失败才允许新尝试。
- 命令运行恢复：自定义命令的回复、@、禁言和跨群发送执行完成后，运行记录落库失败会由受生命周期管理的 worker 重试同一脱敏记录，不重复任何命令动作；MySQL JSON 列的幂等判断使用结构化语义比较，数据库规范化 JSON 后仍能重放首次结果，参数或动作不同则返回冲突。
- 运行时：`internal/management` 组合全部服务，`cmd/bot` 接入 Admin HTTP、自动审批、命令 Registry、Telemetry 与维护循环；Docker 启动前执行迁移，并提供独立管理员引导命令。
- 包结构：Bot 的 `internal/` 只保留 management、automation、groups、knowledge、messaging、bot、ai 和 platform 八个一级功能边界；包路径、测试中的仓库资源定位和全部文档引用已同步迁移。
...
- 真实 MySQL：`go test -race -count=1 ./internal/platform/database`（362.462s）与 `go test -race -count=1 ./internal/platform/storage`（128.974s）分别通过，覆盖 001-009 迁移恢复、管理事务、正式/测试定时运行重试、审批与知识库终态恢复、自定义命令 JSON 幂等重放、业务遥测聚合与导出；测试后两类临时 schema 残留数均为 0。
